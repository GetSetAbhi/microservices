...
public class CurrencyConversionController {
	...
	@GetMapping("/test/from/{from}/to/{to}/quantity/{quantity}")
	public CurrencyConversionBean convertTestCurrency(
			@PathVariable String from, 
			@PathVariable String to,
			@PathVariable BigDecimal quantity) {
		...
		// Url is registered through API gateway
		ResponseEntity<CurrencyConversionBean> responseEntity = new RestTemplate().getForEntity(
				"http://localhost:8765/currency-exchange/from/{from}/to/{to}", CurrencyConversionBean.class,
				uriVariables);
